...
Project Structure
This repo is a **Vite SPA** that runs as a **web app** and as an **Electron renderer**. There is **no React Server Components / “use server”** runtime here. Do not introduce RSC patterns or server-only directives.
...
Define props via an interface/type and type them in the function signature. This codebase currently uses `React.FC` in many places; it is allowed. Prefer consistency over mechanical refactors.
Usage
...
Do not add PropTypes alongside TypeScript in this repo unless there is a concrete runtime validation requirement.